...
function startTimer() {
    if (ival == -1) {
        ...
        ival = setInterval(function() {
            ...
            var tds = (now - start < 20000 ? 19 : total + 19) - Math.floor((now - start) / 1000);
            var tdd = 19 - Math.floor(((now - start) / 100) % 10);
            timer.innerHTML = (usedecimal ? tds + '<small>.' + tdd + '</small>' : (tds + 1));
            if (now - start >= (total * 1000) + 20000) {
                if (useabcd && !oncd) {
                    beep2.play();
                    started = false;
                    lowtime = false;
